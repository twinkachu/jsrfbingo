...
document.addEventListener("DOMContentLoaded", () => {
    peer = new Peer(null, {});
    peer.on('open', (id) => {
		document.getElementById("peersts").innerText = "PeerJS Status: Connected";
        console.log("Peer is ready! ID:", id);
    });

    peer.on('error', (err) => {
		document.getElementById("peersts").innerText = "PeerJS Status: Error, See Console / Contact Kev";
        console.error("PeerJS error:", err);
    });

    const kevingobtn = document.getElementById("kevingobtn")
	const kevangobtn = document.getElementById("kevangobtn")
	const kevongobtn = document.getElementById("kevongobtn")
	const kevungobtn = document.getElementById("kevungobtn")

    const kevingostatus = document.getElementById("kevingostatus");

    const textArea1 = document.getElementById("multiroom1");
    const textArea2 = document.getElementById("multiroom2");

    const multibtn = document.getElementById("multibtn");
    const multistatus1 = document.getElementById("multistatus1");
    const multistatus2 = document.getElementById("multistatus2");
    
    kevingobtn.addEventListener("click", () => {
        if (!socket) {
            socket = new WebSocket("wss://chit.kevcyg.net");

            socket.onopen = () => {
                console.log("Connected to WebSocket server");
                socket.send(JSON.stringify({ username : "automarker" }));
                kevingostatus.innerHTML = "Status : Connected";
                kevingobtn.innerHTML = "Disconnect from Bingo";
	...
	kevangobtn.addEventListener("click", () => {
        if (!socket) {
            ...
            socket.onopen = () => {
                ...
                socket.send(JSON.stringify({ username : "automarker" }));
                kevingostatus.innerHTML = "Status : Connected";
                kevangobtn.innerHTML = "Disconnect from Bango";
                textArea1.disabled = false;
                textArea2.disabled = false;
                multibtn.disabled = false;
	...
	kevongobtn.addEventListener("click", () => {
        if (!socket) {
            socket = new WebSocket("wss://chot.kevcyg.net");

            socket.onopen = () => {
                console.log("Connected to WebSocket server");
                socket.send(JSON.stringify({ username : "automarker" }));
                kevingostatus.innerHTML = "Status : Connected";
                kevongobtn.innerHTML = "Disconnect from Bongo";
                textArea1.disabled = false;
                textArea2.disabled = false;
                multibtn.disabled = false;
	...
	kevungobtn.addEventListener("click", () => {
        if (!socket) {
            socket = new WebSocket("wss://chut.kevcyg.net");

            socket.onopen = () => {
                console.log("Connected to WebSocket server");
                socket.send(JSON.stringify({ username : "automarker" }));
                kevingostatus.innerHTML = "Status : Connected";
                kevungobtn.innerHTML = "Disconnect from Bungo";
                textArea1.disabled = false;
                textArea2.disabled = false;
                multibtn.disabled = false;
    ...
    multibtn.addEventListener("click", () => {
        let multilink1 = textArea1.value.trim();
        let multilink2 = textArea2.value.trim();
        ...
        if (multilink1.includes("https://jsrfmulti.surge.sh/bingo/?connect=")) {
            ...
            if (connectId1) {
                ...
                multistatus1.innerHTML = "Team 1 : Connecting...";
            } else {
                multistatus1.innerHTML = "Team 1 : Invalid link format";
                return;
            }
        } else {
            multistatus1.innerHTML = "Team 1 : Enter a valid link for team 1";
            return;
        }
        ...
        if (multilink2 === "") {
            ...
            if (connectId2) {
                ...
                multistatus2.innerHTML = "Team 2 : Connecting...";
            } else {
                multistatus2.innerHTML = "Team 2 : Invalid link format";
                return;
            }
        } else {
            multistatus2.innerHTML = "Team 2 : Enter a valid link for team 2";
            return;
        }
...
function HandleGraffitiCompletion(levelObj, playerIndex, teamObj) {
	...
	socket.send(JSON.stringify(new BingoEvent("Graf", teamObj.players[playerIndex].name, levelObj.name)))
}
...
function HandleAreaChange(levelID, playerIndex, teamObj) {
	if (playerIndex != undefined) 
	{
		...
		socket.send(JSON.stringify({"type" : "location_update", data : {"name" : teamObj.players[playerIndex].name, "location" : GetLevelFromID(levelID)}}));
